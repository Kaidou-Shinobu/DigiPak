...
void NCGRPak::populate(std::ifstream& inputFILE) {
	...
	for (unsigned int i = 0; i < header.numOfFiles; i++) {
		...
		inputFILE.read(reinterpret_cast<char*>(&file.sizeUncompressed),		sizeof(file.sizeUncompressed));
		inputFILE.read(reinterpret_cast<char*>(&file.sizeNoFileEndPadding),	sizeof(file.sizeNoFileEndPadding));
	...
	for (auto& graphic : graphics) {
		...
		inputFILE.read(graphic.data.get(), graphic.sizeWithFileEndPadding);
	}
...
std::unique_ptr<char[]> NCGRPak::decompressPrototype(NCGRFile& compressedFile) {
	...
	char* compressedData = compressedFile.data.get();
