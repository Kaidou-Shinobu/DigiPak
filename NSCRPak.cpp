...
void NSCRPak::populate(std::ifstream& inputFILE) {
...
void NSCRPak::exportFile(int f) {

}
void NSCRPak::exportAll() {
	std::ofstream outputNCLRFILE;
	for (unsigned int i = 0; i < files.size(); i++) {
		outputNCLRFILE.open("data/nscr/bg_" + std::to_string(i) + ".nscr", std::ios::binary);
